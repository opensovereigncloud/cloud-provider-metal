...
func (o *metalInstancesV2) InstanceMetadata(ctx context.Context, node *corev1.Node) (*cloudprovider.InstanceMetadata, error) {
	...
	if providerID == "" {
		providerID = o.getProviderIDForSeverClaim(serverClaim)
	}
...
func (o *metalInstancesV2) getProviderIDForSeverClaim(serverClaim *metalv1alpha1.ServerClaim) string {
	return fmt.Sprintf("%s://%s/%s/%s", ProviderName, o.metalNamespace, serverClaim.Name, serverClaim.UID)
}
